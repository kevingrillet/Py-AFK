...
class HandlerConfig(metaclass=MetaSingleton):
    ...
    def create_default(self):
        ...
        if not os.path.isfile(self.path):
            self.set_value('debug', str(constant.DEBUG_LEVEL.value), 'Settings')
            self.set_value('dev', str(constant.DEV_MODE), 'Settings')
            self.set_value('scale', str(constant.SCALE), 'Settings')
